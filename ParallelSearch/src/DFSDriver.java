...
public class DFSDriver 
{
    ...
    private void process() throws Exception
    {
        ...
        for(int i = 1; i < 9; i++)
            for(int j = 1; j < 8; j++)
            {
                ...
                doMultiThread(out2, i, j);
                f.close();
                //doSingleThread(out, i, j);
                //f.close();
            }
    ...
    private void doMultiThread(File out, int branch, int depth) throws Exception
    {
            if(out.exists())
    ...
    private void doSingleThread(File out, int branch, int depth) throws Exception
    {
        if(out.exists())
            f = new FileWriter(out, true);
        else
